...
type OrderRepositoryTestSuite struct {
	suite.Suite
	Db *sql.DB
	//Cria setup de testes "locais" para não precisar de fato fazer isso no banco
}
...
func (suite *OrderRepositoryTestSuite) TestGivenAnOrder_WhenSave_ThenShouldSaveOrder() {
	...
	//Faz a conexão com o banco e espera que não tenha erro
	err = repo.Save(order)
	suite.NoError(err)

	//Cria variavel vazia
	var orderResult entity.Order
	//Realiza operacao no banco de dados e o SCAN envia os valores para o local destinado
